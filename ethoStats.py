...
# Function to get the details of the videos of a Channel
def get_channel_videos(youtube, channel_id, max_results):
...
def parse_and_update_sheet(youtube, videos_response, next_upload_time, series_dict, worksheet):
    for item in videos_response['items']:
        ...
        if view_count and likes:
            views_likes_ratio = view_count / likes
        else:
...
def main():
    ...
    channel_response = get_channel_details(youtube, "<Channel Name>")
    channel_id = channel_response['items'][0]['id']
    
    videos_response = get_channel_videos(youtube, channel_id, 64)

    sh = open_google_sheets('<SERVICE_ACCOUNT_JSON>', "<GOOGLE_SHEETS_FILENAME>")
